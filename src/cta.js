...
//   const [handleAbrirCheckout, setHandleAbrirCheckout] = useState(false);
//   const [lidarComPagamento, setLidarComPagamento] = useState(false);
//   const [valorPagamento, setValorPagamento] = useState(19.9);
...
//       onPagamentoConfirmado(true);
//       setMostrarCheckout(false);
//     };

//     const lidarComCheckout = () => {
...
//                     {onPagamentoConfirmado ? (<> <p>Receaba</p>
//                     {!mostrarCheckout && (
//                       <Button onClick={handleAbrirCheckout}> Recebar agora </Button>
//                     )}
//                     {mostrarCheckout && (
//                       <CheckoutBricks onPaymentSucess = {lidarComPagamento} />
//                     )}
//                     </>
//                     ) : (
//                       <Button onClick={handleAbrirCheckout}> Recebar agora </Button>
...
//             }}
//           >
//             Adquirir por R$ {valorPagamento.toFixed(2)}
//           </Button>
//         </section>
...
export default function Home() {
  ...
  const [reportsCount] = useState(2000);
  const [externalReference, setExternalReference] = useState(null);
  const [valorPagamento, setValorPagamento] = useState(0.29);
  const [onPagamentoConfirmado, setOnPagamentoConfirmado] = useState(false);
  const [mostrarCheckout, setMostrarCheckout] = useState(false);
  ...
  function validarEntradasCalc() {
    ...
    if (Number.isNaN(a) || a < 5 || a > 120)
      return alert("Idade inválida (5 a 120 anos).");
    if (Number.isNaN(w) || w < 30 || w > 500)
      return alert("Peso inválido (30 a 500 kg).");
    if (Number.isNaN(h) || h < 50 || h > 250)
      return alert("Altura inválida (50 a 250 cm).");
  ...
  function goToCalculo() {
    calculoRef.current?.scrollIntoView({ behavior: "smooth" });
    setTimeout(() => calculoRef.current?.querySelector("input,select")?.focus(), 300);
  }
  ...
  async function handleSendPaid(e) {
    ...
    try {
      ...
      if (res.ok && data?.checkoutUrl) {
        ...
        alert("✅ Pagamento processado. Verifique seu e-mail!");
      }
    } catch {
  ...
  // 🔹 Controladores visuais do checkout
  function handleAbrirCheckout() {
    setMostrarCheckout(true);
  }
  function lidarComPagamento() {
    setOnPagamentoConfirmado(true);
    setMostrarCheckout(false);
  }
  ...
  return (
    <div className="bg-light min-vh-100 py-5 d-flex flex-column align-items-center">
      <div className="container bg-white shadow rounded-4 p-4 p-md-5">
        ...
        {/* SEÇÕES DE BENEFÍCIOS, PROVA SOCIAL E OFERTA mantidas */}
